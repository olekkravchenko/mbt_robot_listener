...
class MBTGeneratorListener(object):
    ...
    def overwrite_output_file(self):
        ...
        for test in suite.findall('test[@name="{remove_name}"]'.format(remove_name=self.REMOVE_NAME)):
            suite.remove(test)
        for msg in errors.findall('msg'):
            if self.REMOVE_NAME in msg.text:
                errors.remove(msg)
        tree.write(self.output_file_path)

    def overwrite_report_file(self):
        """Method overwrites report.html file created by robot executor after output.xml file is changed

        :return:
        """
        writer = ResultWriter(self.output_file_path)
        writer.write_results(report=self.report_file_path)
